const yearEl = document.getElementById("year");
if (yearEl) yearEl.textContent = String(new Date().getFullYear());

const fullImageEl = document.getElementById("fullImage");
const fullImageTitleEl = document.getElementById("fullImageTitle");
const fullImageCaptionEl = document.getElementById("fullImageCaption");

if (fullImageEl) {
	const params = new URLSearchParams(window.location.search);
	const img = params.get("img") || "assets/Landing page.jpg";
	const title = params.get("title") || "Full Image";

	fullImageEl.src = img;
	fullImageEl.alt = `${title} full-size image`;

	if (fullImageTitleEl) fullImageTitleEl.textContent = title;
	if (fullImageCaptionEl) fullImageCaptionEl.textContent = img;

	document.title = `${title} | Senior Design`;
}

document.querySelectorAll(".shot").forEach((shot) => {
	const img = shot.querySelector("img");
	const ph = shot.querySelector(".shot-ph");
	if (!img) return;

	img.addEventListener("error", () => {
		img.style.display = "none";
		shot.classList.add("missing");
		if (ph) ph.style.display = "grid";
		shot.removeAttribute("href");
		shot.removeAttribute("target");
		shot.removeAttribute("rel");
	});

	img.addEventListener("load", () => {
		if (ph) ph.style.display = "none";
		shot.classList.remove("missing");
	});
});

const SEED = Math.floor(Math.random() * 1e9);

(() => {
	const canvas = document.getElementById("bgNoise");
	if (!canvas) return;

	const ctx = canvas.getContext("2d", { alpha: false });
	if (!ctx) return;

	const FPS = 10;
	const FRAME_MS = 1000 / FPS;

	const MAX_W = 260;
	const MAX_H = 160;

	function mulberry32(seed) {
		return function () {
			let t = (seed += 0x6D2B79F5);
			t = Math.imul(t ^ (t >>> 15), t | 1);
			t ^= t + Math.imul(t ^ (t >>> 7), t | 61);
			return ((t ^ (t >>> 14)) >>> 0) / 4294967296;
		};
	}

	const fade = (t) => t * t * t * (t * (t * 6 - 15) + 10);
	const lerp = (a, b, t) => a + (b - a) * t;

	function makePerm(seed) {
		const rand = mulberry32(seed);
		const p = new Uint8Array(256);
		for (let i = 0; i < 256; i++) p[i] = i;
		for (let i = 255; i > 0; i--) {
			const j = Math.floor(rand() * (i + 1));
			const tmp = p[i];
			p[i] = p[j];
			p[j] = tmp;
		}
		const perm = new Uint8Array(512);
		for (let i = 0; i < 512; i++) perm[i] = p[i & 255];
		return perm;
	}

	function grad(hash, x, y) {
		switch (hash & 7) {
			case 0: return  x + y;
			case 1: return -x + y;
			case 2: return  x - y;
			case 3: return -x - y;
			case 4: return  x;
			case 5: return -x;
			case 6: return  y;
			default: return -y;
		}
	}

	function perlin2(perm, x, y) {
		const X = Math.floor(x) & 255;
		const Y = Math.floor(y) & 255;

		const xf = x - Math.floor(x);
		const yf = y - Math.floor(y);

		const u = fade(xf);
		const v = fade(yf);

		const aa = perm[perm[X] + Y];
		const ab = perm[perm[X] + Y + 1];
		const ba = perm[perm[X + 1] + Y];
		const bb = perm[perm[X + 1] + Y + 1];

		const x1 = lerp(grad(aa, xf, yf),     grad(ba, xf - 1, yf),     u);
		const x2 = lerp(grad(ab, xf, yf - 1), grad(bb, xf - 1, yf - 1), u);

		return (lerp(x1, x2, v) + 1) * 0.5;
	}

	function fbm(perm, x, y) {
		let value = 0;
		let amp = 1;
		let freq = 5;
		for (let i = 0; i < 4; i++) {
			value += amp * perlin2(perm, x * freq, y * freq);
			freq *= 2.0;
			amp *= 0.5;
		}
		return value;
	}

	const perm = makePerm(SEED);

	let img = null;
	let data = null;
	let baseX = null;
	let baseY = null;

	const base = [7, 9, 13];
	const light = [26, 30, 38];

	const strength = 0.65;
	const cutoff = 0.70;
	const gamma = 2.1;

	const scale = 0.0006;
	const speed = 0.006;
	const driftX = 0.22;
	const driftY = 0.16;

	const hueDX = -0.12;
	const hueDY = 0.20;

	const clamp255 = (v) => (v < 0 ? 0 : v > 255 ? 255 : v);
	const saturate = (v) => (v < 0 ? 0 : v > 1 ? 1 : v);
	const smoothstep = (a, b, x) => {
		const t = saturate((x - a) / (b - a));
		return t * t * (3 - 2 * t);
	};

	let pxScale = 1;

	function resize() {
		const dpr = Math.max(1, Math.min(2, window.devicePixelRatio || 1));
		const targetW = Math.floor(window.innerWidth * dpr);
		const targetH = Math.floor(window.innerHeight * dpr);

		const s = Math.min(1, MAX_W / targetW, MAX_H / targetH);

		pxScale = dpr * s;

		canvas.width = Math.max(2, Math.floor(targetW * s));
		canvas.height = Math.max(2, Math.floor(targetH * s));
		canvas.style.width = "100vw";
		canvas.style.height = "100vh";

		img = ctx.createImageData(canvas.width, canvas.height);
		data = img.data;

		baseX = new Float32Array(canvas.width);
		baseY = new Float32Array(canvas.height);
		for (let x = 0; x < canvas.width; x++) baseX[x] = x * scale;
		for (let y = 0; y < canvas.height; y++) baseY[y] = y * scale;

		drawFrame();
	}

	let needsRedraw = true;
	let lastScrollY = window.scrollY;

	function drawFrame() {
		const scrollNoise = (window.scrollY * pxScale) * scale;

		const w = canvas.width;
		const h = canvas.height;

		let idx = 0;
		for (let y = 0; y < h; y++) {
			const ny = baseY[y] + scrollNoise + t * driftY;
			const hy = baseY[y] + scrollNoise + t * hueDY;

			for (let x = 0; x < w; x++) {
				const nx = baseX[x] + t * driftX;
				const hx = baseX[x] + t * hueDX;

				const n = fbm(perm, nx, ny);
				let m = smoothstep(cutoff, 0.98, n);
				m = Math.pow(m, gamma);

				const hN = fbm(perm, hx + 100.0, hy - 50.0);
				const rr = lerp(base[0], light[0], hN);
				const gg = lerp(base[1], light[1], hN);
				const bb = lerp(base[2], light[2], hN);

				const r = lerp(base[0], rr, m * strength);
				const g = lerp(base[1], gg, m * strength);
				const b = lerp(base[2], bb, m * strength);

				data[idx++] = clamp255(r);
				data[idx++] = clamp255(g);
				data[idx++] = clamp255(b);
				data[idx++] = 255;
			}
		}

		ctx.putImageData(img, 0, 0);
	}

	let lastTS = 0;
	let accMS = 0;
	let t = 0;

	let scrollBoostUntil = 0;

	function tick(ts) {
		if (!img) return;

		if (!lastTS) lastTS = ts;
		const dtMS = Math.min(80, ts - lastTS);
		lastTS = ts;

		const y = window.scrollY;
		if (y !== lastScrollY) {
			lastScrollY = y;
			scrollBoostUntil = ts + 140;
			needsRedraw = true;
		}

		const scrollingNow = ts < scrollBoostUntil;

		if (scrollingNow) {
			t += (dtMS / 1000) * speed;
			drawFrame();
			needsRedraw = false;
		} else {
			accMS += dtMS;
			if (accMS >= FRAME_MS || needsRedraw) {
				t += (accMS / 1000) * speed;
				accMS = 0;
				drawFrame();
				needsRedraw = false;
			}
		}

		requestAnimationFrame(tick);
	}

	window.addEventListener("resize", resize, { passive: true });
	resize();
	requestAnimationFrame(tick);
})();
